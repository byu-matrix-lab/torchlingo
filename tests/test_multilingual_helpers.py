...
class TestGetLanguageTagsFromVocab(unittest.TestCase):
    ...
    def test_extracts_uppercase_bidirectional_tags(self):
        """Test extracting the config-default bidirectional tags <2X>/<2E>."""
        vocab = SimpleVocab(min_freq=1)
        vocab.build_vocab(["<2X> hello world", "<2E> hola mundo"])

        tags = get_language_tags_from_vocab(vocab)

        self.assertIn("<2X>", tags)
        self.assertIn("<2E>", tags)
...
class TestDefaultLanguageTags(unittest.TestCase):
    ...
    def test_all_tags_follow_pattern(self):
        ...
        for tag in DEFAULT_LANGUAGE_TAGS.values():
            self.assertTrue(
                pattern.match(tag), f"Tag '{tag}' does not match expected pattern <2XX>"
            )
